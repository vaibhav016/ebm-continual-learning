...
def train_cl_decoupled(args, model, train_datasets, scenario="class",labels_per_task=None,iters=2000,batch_size=32,
             loss_cbs=list(), eval_cbs=list()):
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        # Wake phase(considering only positive energy)
        print("\n-------Wake Phase--------")
        iters_left = 1
        for batch_index in range(1, iters+1):
            iters_left -= 1
            ...
            if batch_index <= iters:
                ...
                if not decoupled_contrastive_loss:
                    loss_dict = model.train_a_batch(args, x, y, x_, y_, task=task, device=device)
        ...
        ## -------------------------------------------------------------------------------------------------------------
        # Sleep Phase(considering both positive and negative energy)
        print("\n-------Sleep Phase--------")
        iters_left = 1
        for batch_index in range(1, iters+1):
            iters_left -= 1
            ...
            if batch_index <= iters:
                ...
                if not decoupled_contrastive_loss:
                    ...
                    # print("decoupled training")
                    loss_dict = model.train_a_batch_decoupled_contrastive(args, x, y, x_, y_, task=task, device=device, energy_negative=True,
                                                                          energy_positive=False, previous_loss=0,
